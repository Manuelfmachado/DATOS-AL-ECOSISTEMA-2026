...
# Offline: los CSVs viven en alba-offline/data/processed (parents[3] desde routers/).
...
def _load_json_optional(name: str) -> dict:
    """Como _load_json pero devuelve {} si el archivo no existe (no lanza 503)."""
    path = DATA / name
    if not path.exists():
        return {}
    with open(path, "r", encoding="utf-8") as f:
...
async def sim_demanda_sectorial(req: EscenarioRequest):
    ...
    # Cargar predicción GEIH del sector (opcional: no todos los deployments generan predicciones_geih.json)
    pred_geih = _load_json_optional("predicciones_geih.json")
    ...
    if len(proyeccion_base) < 12:
        # Sin predicción Chronos mensual: línea base plana en el empleo actual
        proyeccion_base = [empleo_actual] * 12
